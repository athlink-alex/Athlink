...
const coachNav: NavItem[] = [
  ...
  { label: 'Analytics', to: '/dashboard/coach/analytics', icon: 'M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z', tier: 'elite' },
  { label: 'Reports', to: '/dashboard/coach/reports', icon: 'M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z', tier: 'elite' },
...
function LockIcon() {
  return (
    <svg className="w-3 h-3 text-gray-400 dark:text-gray-500 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
export function Sidebar() {
  ...
  // Fetch count of past sessions without reports for coach notification dot
  useEffect(() => {
    ...
    const fetchUnreported = async () => {
      ...
      // Get all past bookings for this coach
      ...
      // Get bookings that already have reports
  ...
  // Inject badge count into coach nav items
  const navItems = (() => {
  ...
  return (
    <aside className="hidden md:flex md:w-56 md:flex-col md:border-r md:border-[#E5E7EB] dark:md:border-gray-800 bg-white dark:bg-gray-950 min-h-[calc(100vh-4rem)] transition-colors duration-200">
            ...
            <Link
              ...
              className={`flex items-center gap-3 rounded-[8px] px-3 py-2.5 text-sm font-medium transition-colors ${
                isActive
                  ...
                  : 'text-gray-600 dark:text-gray-400 hover:bg-[#F9FAFB] dark:hover:bg-gray-800 hover:text-[#2563EB]'
              }`}
            >
